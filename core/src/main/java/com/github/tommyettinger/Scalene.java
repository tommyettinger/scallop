...
public class Scalene extends ApplicationAdapter {
	public Array<String> files;
	public PaletteReducer palette;
	public AtomicInteger currentlyRunning;
	public Scalene(String[] filenames, AtomicInteger running) {
...
//		return (L * L + A * A + B * B) >= 0.01;
		...
		public static void scale2p(ByteBuffer dest, int A, int B, int C, int D, int E, int F, int G, int H, int I, int p0, int p1,
							   int p2, int p3) {
		dest.putInt(p0, D != 0 && ((!different(D, B) && (different(B, F) && different(F, H))) || (E == 0 && (B != 0 || H != 0))) ? D : E);
		dest.putInt(p1, B != 0 && ((!different(B, F) && (different(B, H) && different(H, D))) || (E == 0 && (F != 0 || D != 0))) ? B : E);
		dest.putInt(p2, H != 0 && ((!different(H, D) && (different(D, B) && different(B, F))) || (E == 0 && (D != 0 || F != 0))) ? H : E);
		dest.putInt(p3, F != 0 && ((!different(F, H) && (different(B, D) && different(D, H))) || (E == 0 && (H != 0 || B != 0))) ? F : E);
	}
